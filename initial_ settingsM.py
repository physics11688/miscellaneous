...
freeze_output = subprocess.run(
    "pip3 freeze", capture_output=True, text=True, shell=True
).stdout
if not ("PySimpleGUI" in freeze_output):
    subprocess.run(
        "pip3 install FreeSimpleGUI --break-system-packages", shell=True
    )  # 無ければインストール
...
# アーキテクチャの確認
arch = subprocess.run(
    "arch", capture_output=True, text=True, shell=True
).stdout.replace("\n", "")
...
def Homebrew():
    ...
    if os.path.exists(path_to_brew):
        ...
        comment = (
            "Homebrewはインストールされていなかったので,インストールしておきました."
        )
...
layout = [
    ...
    [
        sg.Checkbox(
            "Homebrewインストール後の【 echo 'export PATH???】はちゃんと入力できてる？",
            font=("Arial", 15),
            key="Homebrewecho",
        )
    ],
